...
void KittiPublishersNode::on_timer_callback()
{
  ...
  {
      RCLCPP_INFO(this->get_logger(), "Publishing message %lu", file_index_);
      ...
      //RCLCPP_INFO(this->get_logger(), "OxTs size: '%lu'", oxts_parsed_array.size());
      ...
      RCLCPP_INFO(this->get_logger(), "No more data: all messages have been published.");
    }
}
...
void KittiPublishersNode::init_file_path()
{
    ...
    this->declare_parameter("kitti_prefix", rclcpp::ParameterType::PARAMETER_STRING);
    this->get_parameter("kitti_prefix", param);
    std::string prefix = param.as_string();
    this->declare_parameter("kitti_date", rclcpp::ParameterType::PARAMETER_STRING);
    this->get_parameter("kitti_date", param);
    std::string date = param.as_string();
    this->declare_parameter("kitti_dataset", rclcpp::ParameterType::PARAMETER_STRING);
    this->get_parameter("kitti_dataset", param);
    std::string dataset = param.as_string();

    std::string path = prefix + "/" + date + "/" + dataset;
    RCLCPP_INFO(this->get_logger(), "Loading data at: %s", path.c_str());
 
    path_point_cloud_ = path + "/velodyne_points/data/";
    path_image_gray_left_ = path +  "/image_00/data/";
    path_image_gray_right_ = path + "/image_01/data/";
    path_image_color_left_ = path + "/image_02/data/";
    path_image_color_right_ = path + "/image_03/data/";
    path_oxts_ = path + "/oxts/data/";
}
